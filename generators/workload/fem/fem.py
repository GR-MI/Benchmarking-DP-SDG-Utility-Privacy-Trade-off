...
def generate(real_answers:np.array,
    ...
    for t in range(T):
        ...
        EM_dist_0 = np.exp(epsilon_0 * score * N / 2, dtype=np.float64)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, help='queries')
    parser.add_argument('--workload', type=int, default=32, help='queries')
    parser.add_argument('--marginal', type=int, default=3, help='queries')
    ...
    dataset = args.dataset  # Should be a string path
    workload = args.workload  # Should be an integer
    marginal = args.marginal  # Should be an integer
    epsilon = args.epsilon  # List of epsilon values
    ######################################################
    ## Get dataset
    ######################################################
    data, workloads = benchmarks.randomKway(args.dataset, domain_path, args.workload, args.marginal)
    N = data.df.shape[0]
    delta = 1.0/N**2
    ...
    for eps in args.epsilon:
        ...
        temp = [args.dataset, len(query_manager.queries), args.workload, args.marginal, 'FEM', eps,
                max_error,
                rmse,
                fem_runtime,
    ...
    #synthetic data
    final_synthetic_data_path = output_file
    fem_data.df.to_csv(final_synthetic_data_path, index=False)
    print(f"Final synthetic data saved at: {final_synthetic_data_path}")

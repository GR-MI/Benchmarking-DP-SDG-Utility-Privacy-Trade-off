...
class CliqueVector:
    ...
    def parent(self, clique: Clique) -> Union[Clique, None]:
    ...
    def __add__(self, other: Union[chex.Numeric, "CliqueVector"]) -> "CliqueVector":
        if isinstance(other, CliqueVector):
            return jax.tree.map(jnp.add, self, other)
        return jax.tree.map(lambda f: f + other, self)

    def __sub__(self, other: Union[chex.Numeric, "CliqueVector"]) -> "CliqueVector":
        return self + -1 * other

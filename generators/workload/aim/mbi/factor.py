...
class Factor:
    ...
    def _aggregate(
        self, fn: Callable, attrs: Union[Collection[str], None] = None
    ) -> "Factor":
    ...
    def max(self, attrs: Union[Collection[str], None] = None) -> "Factor":
        return self._aggregate(jnp.max, attrs)

    def sum(self, attrs: Union[Collection[str], None] = None) -> "Factor":
        return self._aggregate(jnp.sum, attrs)

    def logsumexp(self, attrs: Union[Collection[str], None] = None) -> "Factor":
        return self._aggregate(jax.scipy.special.logsumexp, attrs)

    def project(self, attrs: Union[str, tuple[str, ...]], log: bool = False) -> "Factor":
    ...
    def _binaryop(self, fn: Callable, other: Union["Factor", chex.Numeric]) -> "Factor":
    # Check if 'other' is a scalar (either int, float or jnp.ndarray with 0 dimensions)
        if isinstance(other, (float, int)) or (isinstance(other, jnp.ndarray) and jnp.ndim(other) == 0):
        # If 'other' is a scalar, wrap it in a Factor with an empty domain
            other = Factor(Domain([], []), other)
    
    # Merge the domains of the two factors
        newdom = self.domain.merge(other.domain)
        
        # Expand both factors to the new domain
        factor1 = self.expand(newdom)
        factor2 = other.expand(newdom)
    
    # Apply the function `fn` to the values of the two expanded factors
        return Factor(newdom, fn(factor1.values, factor2.values))


    def __sub__(self, other: Union["Factor", chex.Numeric]) -> "Factor":
        return self._binaryop(jnp.subtract, other)

    def __truediv__(self, other: Union["Factor", chex.Numeric]) -> "Factor":
        return self._binaryop(jnp.divide, other)

    def __mul__(self, other: Union["Factor", chex.Numeric]) -> "Factor":
    ...
    def __add__(self, other: Union["Factor", chex.Numeric]) -> "Factor":
